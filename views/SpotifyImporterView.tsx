...
import { searchSpotifyArtists, getArtistAlbums, SpotifyArtistSimple, SpotifyAlbumSimple } from '../services/spotifyService';
import type { YouTubeMusicAlbumSimple } from '../types';
...
interface SpotifyImporterViewProps {
    addMultipleSongsFromImport: (albums: YouTubeMusicAlbumSimple[]) => void;
}

const SpotifyImporterView: React.FC<SpotifyImporterViewProps> = ({ addMultipleSongsFromImport }) => {
    const navigate = useNavigate();
    const [searchTerm, setSearchTerm] = useState('');
    const [viewState, setViewState] = useState<ViewState>('search_artist');
    
    const [artists, setArtists] = useState<SpotifyArtistSimple[]>([]);
    const [albums, setAlbums] = useState<SpotifyAlbumSimple[]>([]);
    const [selectedArtist, setSelectedArtist] = useState<SpotifyArtistSimple | null>(null);
    ...
    const handleSearch = async (e: React.FormEvent) => {
        ...
        const searchResults = await searchSpotifyArtists(searchTerm);
        setArtists(searchResults);
        setIsLoading(false);
    };

    const handleSelectArtist = async (artist: SpotifyArtistSimple) => {
        setIsLoading(true);
    ...
    const handleImportSelected = () => {
        ...
        if (albumsToImport.length > 0) {
            const youtubeLikeAlbums: YouTubeMusicAlbumSimple[] = albumsToImport.map(album => ({
                id: album.id,
                name: album.name,
                album_type: album.album_type === 'compilation' ? 'album' : album.album_type,
                artists: album.artists,
                thumbnails: album.images.map(img => ({ url: img.url, width: img.width, height: img.height })),
                release_date: album.release_date,
            }));
            addMultipleSongsFromImport(youtubeLikeAlbums);
            navigate('/releases');
        }
    };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {artists.map(artist => (
                <div key={artist.id} onClick={() => handleSelectArtist(artist)} className="group relative bg-ui-surface rounded-2xl p-4 text-center cursor-pointer card-interactive-glow overflow-hidden">
                    <img src={artist.images[0]?.url || `https://picsum.photos/seed/${artist.id}/300`} alt={artist.name} className="w-32 h-32 rounded-full object-cover mb-4 mx-auto shadow-lg transition-transform duration-300 group-hover:scale-105" />
                ...
                {albums.map(album => (
                    <div key={album.id} onClick={() => handleToggleAlbumSelection(album.id)} className={`bg-ui-surface rounded-xl border-2 p-4 flex items-center gap-4 transition-all duration-300 cursor-pointer ${selectedAlbums.has(album.id) ? 'border-primary' : 'border-ui-border hover:border-primary/50'}`}>
                         <input
                            ...
                            className="h-5 w-5 rounded text-primary focus:ring-primary bg-ui-bg border-ui-border"
                        />
                        <img src={album.images[0]?.url} alt={album.name} className="w-16 h-16 rounded-lg object-cover" />
        ...
        <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
            <div>
                ...
                <h1 className="text-4xl font-bold text-ui-text-heading flex items-center gap-3">
                    ...
                    Spotify Release Importer
                </h1>
                <p className="text-lg text-ui-text-body mt-1">Search for an artist to import album or single metadata and artwork.</p>
            </div>
...
export default SpotifyImporterView;